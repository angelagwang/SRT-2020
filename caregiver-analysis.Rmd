...
phq9 <- as.tibble(phq9)
phq9<- phq9 %>%
  mutate(`Depression Severity` = fct_relevel(`Depression Severity`, levels = "Minimal Depression", "Mild depression", "Moderate depression", "Moderately severe depression"))
...
## Regression Output
```{r}
phq9model <- lm(phq9$`Total: Used to see severity of depressive disorder` ~ phq9$`Age` + phq9$`Gender` + phq9$`Time in Ghar`, data = phq9)
...
## Regression Output
```{r}
gad7model <- lm(gad7$`Scores` ~ gad7$`Age` + gad7$`Gender` + gad7$`Time in Ghar`, data = gad7)
...
ggplot(data = obi, aes(x = `Disengagement Mean`)) + 
  geom_histogram(bins = 10,color = "black", fill = "gray") +
  labs(title = "Average Disengagement Scores in Caregivers", x = "Average Disengagement Score", y = "Count")

ggplot(data = obi, aes(x = `Exhaustion Mean`)) + 
  geom_histogram(bins = 10,color = "black", fill = "gray") +
  labs(title = "Average Exhaustion Scores in Caregivers", x = "Average Exhaustion Score", y = "Count")
